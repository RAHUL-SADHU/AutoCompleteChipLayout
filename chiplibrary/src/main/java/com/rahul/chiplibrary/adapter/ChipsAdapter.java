package com.rahul.chiplibrary.adapter;
...
import com.rahul.chiplibrary.ChipView;
import com.rahul.chiplibrary.ChipsInput;
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if(actionId == EditorInfo.IME_ACTION_NEXT){
                    if(mEditText.getText().toString().trim().length()>1){
                        mChipsInput.addChip(mEditText.getText().toString().trim(),"");
                    }
                }
                return false;
            }
